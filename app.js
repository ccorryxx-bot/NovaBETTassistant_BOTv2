...
window.addEventListener('load', () => {
  ...
  addMessage('bot',
    'မင်္ဂလာပါ! NovaBETT Game AI Support မှ ကြိုဆိုပါသည်။\nကျွန်ုပ်တို့ ဘာများ ကူညီပေးရမလဲ?',
    true
  );
});
...
window.saveToCloud = async () => {
  ...
  try {
    ...
    btn.innerHTML = '<svg><use href="#ico-shield"/></svg> Saved';
    btn.classList.add('saved');
    setTimeout(() => {
      btn.innerHTML = '<svg><use href="#ico-upload"/></svg> Push to Cloud';
      btn.classList.remove('saved');
      btn.disabled = false;
    }, 2800);
  } catch(e) {
